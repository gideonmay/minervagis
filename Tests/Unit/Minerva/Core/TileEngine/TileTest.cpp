...
protected:
  virtual void SetUp()
  {
    ...
    // Make a tile.
    Minerva::Common::TileKey::RefPtr tileInfo ( new Minerva::Common::TileKey );
    tileInfo->level ( level );
    tileInfo->extents ( _extents );
    tileInfo->imageSize ( imageSize );
    tileInfo->meshSize ( meshSize );

    _tile = new Tile ( tileInfo, 1.0, _body.get(), 0x0, elevation.get() );
