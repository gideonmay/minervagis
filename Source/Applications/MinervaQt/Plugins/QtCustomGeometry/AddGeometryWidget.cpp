...
#include "Minerva/OsgTools/Torus.h"
...
AddGeometryWidget::AddGeometryWidget ( QWidget *parent ) : BaseClass ( parent ),
...
void AddGeometryWidget::apply ( Minerva::Core::Data::Feature* parent )
{
...
namespace Detail
{
  ...
  {
    osg::ref_ptr<osg::Geode> geode ( new osg::Geode );
    geode->addDrawable ( new osg::ShapeDrawable ( new osg::Box ( osg::Vec3 ( 0.0, 0.0, 0.0 ), w, h ,d ) ) );
    return geode.release();
  }
}
...
{
  const osg::Vec4f color ( Minerva::QtTools::Color<osg::Vec4f>::convert ( _colorButton->color() ) );
