...
public:
  Program( const std::string& program = "VRV" )
  {
    ...
    // Use 2 threads.
    Usul::Jobs::Manager::init( "Job Manager Singleton", 2 );
  }
...
protected:
  ...
  int _run ( int argc, char ** argv )
  {
    ...
    // Read the registry.
    Usul::Functions::safeCall ( boost::bind ( &Program::_readRegistryFile, this ), "3499560042" );

    // Load the plugins. Not a big deal if it throws.
    Usul::Functions::safeCall ( boost::bind ( &Program::_loadPlugins, this ), "1701127221" );
  ...
  void _loadPlugins()
  {
    if ( boost::filesystem::exists ( this->_pluginFileName() ) )
    {
      // Load the plugins.
