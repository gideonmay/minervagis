      ...
      static void convert ( const Blob &from, Usul::Math::Vec2d &to )
      {
        ...
        if ( ThisType::NUM_BYTES != from.size() )
        {
          throw std::invalid_argument ( Usul::Strings::format 
            ( "Error 3366212750: Invalid character vector size ", from.size(), ", expected ", static_cast<int> ( ThisType::NUM_BYTES ) ) );
        }
        ...
        {
          throw std::invalid_argument ( Usul::Strings::format
            ( "Error 2344339624: Invalid geometry type ", type, ", expected ", static_cast<int> ( ThisType::POINT_TYPE ) ) );
        }
  ...
  namespace Convert
  {
    template <> struct Type < std::vector<unsigned char>, std::vector<Usul::Math::Vec2d> >
    {
      ...
      static void convert ( const Blob &from, Line &to )
      {
        ...
        {
          throw std::invalid_argument ( Usul::Strings::format 
            ( "Error 3438257445: Invalid character vector size ", from.size(), ", expected at least ", static_cast<int> ( ThisType::MIN_BYTES ) ) );
        }
        ...
        {
          throw std::invalid_argument ( Usul::Strings::format
            ( "Error 1391184162: Invalid geometry type ", type, ", expected ", static_cast<int> ( ThisType::LINE_TYPE ) ) );
        }
