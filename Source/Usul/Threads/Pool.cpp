...
#pragma warning ( disable : 4244 ) // conversion warning in boost thread sleep function.
...
std::size_t Pool::numThreads() const
{
  Guard guard ( this );
  return _pool.size();
}
...
std::size_t Pool::numTasksQueued() const
{
  Guard guard ( this );
  return _queue.size();
}
...
std::size_t Pool::numTasksExecuting() const
{
  Guard guard ( this );
...
std::size_t Pool::numTasks() const
{
  // Lock so that the answers are in sync.
  Guard guard ( this );

  // Get the numbers.
  const std::size_t queued ( this->numTasksQueued() );
  const std::size_t executing ( this->numTasksExecuting() );
...
void Pool::_threadStarted()
{
  ...
  while ( true == Usul::Threads::Safe::get ( this->mutex(), _runThreads ) )
  {
    ...
    if ( true == task.valid() )
    {
      ...
      Usul::Functions::safeCall ( boost::bind ( &Usul::Threads::Pool::_threadProcessTask, this, task.get() ), "1232600780" );
