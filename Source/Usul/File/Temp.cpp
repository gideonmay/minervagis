...
Temp::~Temp()
{
  try
  {
    ...
    if ( _remove )
    {
      ...
      if ( result != 0 )
      {
        std::cout << "Error 2034122402: Failed to remove temporary file: " + this->name() << std::endl;
      }
    }
  }
  ...
  {
    std::cout << "Error 2355835989: Standard exception caught while removing temporary file: " + this->name() << std::endl;
    std::cout << "Message: " << e.what() << std::endl;
  }

  catch ( ... )
  {
    std::cout << "Error 2036890444: Unknown exception caught while removing temporary file: " + this->name() << std::endl;
  }
}
...
std::string Temp::file()
{
  ...
  // Use a local string to hold the filename.  This is thread-safe (Passing null as agrument to tmpnam is not).
...
void Temp::rename ( const std::string &name )
{
  ...
  boost::filesystem::rename ( this->name(), name );
...
{
  Temp file;
  const std::string dir ( Usul::File::directory ( file.name() ) );
  return dir;
}
