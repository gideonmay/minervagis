...
template < class Container > inline void getStackTrace ( Container &c, CONTEXT& context )
{
  ...
  for ( unsigned int i = 0; i < maxStackDepth; ++i )
  {
    ...
    // Get the name of the function.
    {
      ...
      if ( ::SymGetSymFromAddr64 ( ::GetCurrentProcess(), stackFrame.AddrPC.Offset, &displacement, symbol ) )
      {
        ...
        if ( 0 < ::UnDecorateSymbolName ( symbol->Name, &undecorated[0], static_cast<DWORD> ( undecorated.size() - 1 ), UNDNAME_COMPLETE ) )
        {
...
template < class Container > inline void getStackTrace ( Container &c )
{
  ...
  // Get the context.
  Usul::Diagnostics::Helper::getStackTrace ( c, context );
}
