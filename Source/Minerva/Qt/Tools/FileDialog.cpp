...
FileDialog::FilesResult FileDialog::getLoadFileNames ( QWidget* parent, const std::string &title, const Filters &filters )
{
  ...
  if( false == answer.empty() )
  {
    ...
    std::string directory ( Usul::File::directory ( filename ) );
...
FileDialog::FileResult FileDialog::getSaveFileName ( const std::string &title, const Filters &filters )
{
  return FileDialog::getSaveFileName ( QApplication::activeWindow(), title, filters );
}
