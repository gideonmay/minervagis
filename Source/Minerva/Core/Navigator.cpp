...
Navigator::Navigator ( Body::RefPtr body ) : BaseClass(),
  _camera ( new Minerva::Core::Data::Camera ),
  _elevationSpeed ( 1000 ),
  _pitchSpeed ( 10 ),
  _yawSpeed ( 20 ),
  _body ( body )
{
...
void Navigator::viewMatrix ( const osg::Matrixd& m )
{
  if ( _body.valid() )
  {
    ...
    _camera->viewMatrix ( matrix, _body->landModel() );
  }
}
...
void Navigator::rotate ( const MousePosition& point0, const MousePosition& point1, const osg::Matrixd& projection )
{
  Usul::Math::Vec3d pt0, pt1;
  if ( this->_intersectionPoint ( Usul::Math::Vec2d ( point0[0], point0[1] ), projection, pt0 ) && 
       this->_intersectionPoint ( Usul::Math::Vec2d ( point1[0], point1[1] ), projection, pt1 ) )
  {
    if ( _body.valid() )
    {
...
void Navigator::zoomLOS ( const MousePosition& point0, const MousePosition& point1, const osg::Matrixd& projection )
{
  const double dy ( point1[1] - point0[1] );

  Usul::Math::Vec3d pt0 ( this->_convertMouseToWorldSpace ( Usul::Math::Vec3d ( 0.0, 0.0, -1.0 ), projection ) );
  Usul::Math::Vec3d pt1 ( this->_convertMouseToWorldSpace ( Usul::Math::Vec3d ( 0.0, 0.0, 1.0 ), projection ) );

  Usul::Math::Vec3d direction ( pt0 - pt1 );
  direction.normalize();

  this->zoomAlongDirection ( direction, dy * 10000 );
}
...
void Navigator::look ( double dx, double dy )
{
  ...
  const double pitch ( Usul::Math::clamp ( _camera->tilt() - dy * pitchSpeed, 0.0, 90.0 ) );
  _camera->tilt ( pitch );
}
...
Usul::Math::Vec3d Navigator::_convertMouseToWorldSpace ( const Usul::Math::Vec3d& mouse, const osg::Matrixd& projection ) const
{
  // Get the matrix.
  osg::Matrixd M ( osg::Matrixd::inverse ( this->viewMatrix() ) );
  osg::Matrixd P ( projection );
  osg::Matrixd MP ( M * P );
...
bool Navigator::_intersectionPoint ( const Usul::Math::Vec2d& mouse, const osg::Matrixd& projection, Usul::Math::Vec3d& point )
{
  const double x ( mouse[0] );
  const double y ( mouse[1] );

  Usul::Math::Vec3d pt0 ( this->_convertMouseToWorldSpace ( Usul::Math::Vec3d ( x, y, -1.0 ), projection ) );
  Usul::Math::Vec3d pt1 ( this->_convertMouseToWorldSpace ( Usul::Math::Vec3d ( x, y, 1.0 ), projection ) );
