...
void TiledScreenCapture::operator() ( osg::Image& image, osg::Node* scene, const osg::Matrix& projection )
{
  osg::ref_ptr<osgUtil::SceneView> sceneView ( new osgUtil::SceneView );
  sceneView->setDefaults();
  sceneView->setSceneData ( scene );
  sceneView->setViewMatrix ( this->viewMatrix() );
  sceneView->setProjectionMatrix ( projection );
  sceneView->setViewport ( 0, 0, this->size()[0], this->size()[1] );
...
void TiledScreenCapture::_capturePixels ( osg::Image& image, osgUtil::SceneView& sceneView, const osg::Matrix& projection )
{
  ...
  sceneView.setClearColor ( this->clearColor() );
  double v ( 0.0 );
...
osg::Image* TiledScreenCapture::operator () ( osg::Node* scene, const osg::Matrix& projection )
{
