...
#include "Minerva/Plugins/OSM/XAPIMapQuery.h"
...
void XAPIMapQuery::makeNodesQuery ( Nodes& nodes, Usul::Interfaces::IUnknown::QueryPtr unknown )
{
  ...
  status ( "Downloading" );
  if ( Minerva::Network::downloadToFile ( request, filename ) )
  {
    status ( "Parsing" );
...
void XAPIMapQuery::makeLinesQuery ( Lines& lines, Usul::Interfaces::IUnknown::QueryPtr unknown )
{
  ...
  const bool success ( Minerva::Network::downloadToFile ( request, filename, timeout ) );
...
std::string XAPIMapQuery::_buildRequestUrl ( const std::string& requestType ) const
{
  ...
  const std::string request ( Usul::Strings::format ( _url, "/api/0.6/", requestType, bbox, predicateString ) );
  return request;
}
