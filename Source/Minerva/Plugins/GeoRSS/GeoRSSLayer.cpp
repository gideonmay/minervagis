...
void GeoRSSLayer::_parseItem ( const XmlTree::Node& node )
{
  ...
  boost::posix_time::ptime date ( Minerva::Core::Data::Date::createFromRSS ( pubDate ) );
  ...
  if ( false == imageNode.empty() )
  {
    ...
    std::string filename;
    if ( Minerva::Network::download ( url, filename, true ) )
    {
...
void GeoRSSLayer::_updateLink( Usul::Interfaces::IUnknown* caller )
{
  ...
  Usul::Scope::Caller::RefPtr scope ( Usul::Scope::makeCaller ( boost::bind ( &GeoRSSLayer::downloading, this, false ) ) );
...
void GeoRSSLayer::_addTimer()
{
  ...
  if ( factory.valid() )
  {
    ...
    if ( true == _timer.valid() )
      _timer = 0x0;
    
    // Make a new timer.  The timer expects the timeout to be in milliseconds.
    _timer = factory->createTimer ( Usul::Threads::Safe::get ( this->mutex(), _refreshInterval ) * 1000, boost::bind ( &GeoRSSLayer::_timerNotify, GeoRSSLayer::RefPtr ( this ) ) );
  }
}
...
void GeoRSSLayer::downloadFeed()
{
  ...
  Usul::Jobs::Job::RefPtr job ( Usul::Jobs::create ( 
    boost::bind ( &GeoRSSLayer::_updateLink, GeoRSSLayer::RefPtr ( this ), static_cast<Usul::Interfaces::IUnknown*> ( 0x0 ) ) ) );
