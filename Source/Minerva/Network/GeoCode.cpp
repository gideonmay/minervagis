...
GeoCode::Result GeoCode::operator() ( const std::string& location ) const
{
  ...
  // Make a request.
  std::string request ( Usul::Strings::format ( Detail::requestUrl, "?appid=", this->applicationId(), "&location=", Minerva::Network::Http::encode ( location ) ) );
  
  std::string filename;
  if ( Minerva::Network::download ( request, filename ) )
  {
