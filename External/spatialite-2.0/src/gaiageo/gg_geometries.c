...
GAIAGEO_DECLARE void
gaiaGeometryType (gaiaGeomCollPtr geom, char **result)
{
...
GAIAGEO_DECLARE void
gaiaGeometryAliasType (gaiaGeomCollPtr geom, char **result)
{
...
*/
    ...
    double x;
    double y;
    if (gaiaIntersect (&x, &y, mbr1->MinX, mbr1->MinY, mbr1->MinX, mbr1->MaxY,
		       mbr2->MinX, mbr2->MinY, mbr2->MaxX, mbr2->MinY))
	return 1;
    if (gaiaIntersect (&x, &y, mbr1->MaxX, mbr1->MinY, mbr1->MaxX, mbr1->MaxY,
		       mbr2->MinX, mbr2->MaxY, mbr2->MaxX, mbr2->MaxY))
	return 1;
    if (gaiaIntersect (&x, &y, mbr1->MinX, mbr1->MinY, mbr1->MinX, mbr1->MaxY,
		       mbr2->MinX, mbr2->MaxY, mbr2->MaxX, mbr2->MaxY))
	return 1;
    if (gaiaIntersect (&x, &y, mbr1->MaxX, mbr1->MinY, mbr1->MaxX, mbr1->MaxY,
		       mbr2->MinX, mbr2->MinY, mbr2->MaxX, mbr2->MinY))
	return 1;
    if (gaiaIntersect (&x, &y, mbr1->MinX, mbr1->MinY, mbr1->MaxX, mbr1->MinY,
		       mbr2->MinX, mbr2->MinY, mbr2->MinX, mbr2->MaxY))
	return 1;
    if (gaiaIntersect (&x, &y, mbr1->MinX, mbr1->MaxY, mbr1->MaxX, mbr1->MaxY,
		       mbr2->MaxX, mbr2->MinY, mbr2->MaxX, mbr2->MaxY))
	return 1;
    if (gaiaIntersect (&x, &y, mbr1->MinX, mbr1->MinY, mbr1->MaxX, mbr1->MinY,
		       mbr2->MaxX, mbr2->MinY, mbr2->MaxX, mbr2->MaxY))
	return 1;
...
GAIAGEO_DECLARE void
gaiaBuildMbr (double x1, double y1, double x2, double y2,
...
/* setting the Blob value */
    ...
    *(ptr + 1) = GAIA_LITTLE_ENDIAN;	/* byte ordering */
    gaiaExport32 (ptr + 2, -1, 1, endian_arch);	/* the SRID */
...
GAIAGEO_DECLARE void
gaiaBuildCircleMbr (double x, double y, double radius, unsigned char **result,
		    int *size)
{
/* build a Blob encoded Geometry representing an MBR */
    int sz;
    ...
    gaiaBuildMbr (minx, miny, maxx, maxy, &res, &sz);
    if (!res)
      {
	  *result = NULL;
	  *size = 0;
